...
Base_Driver::Base_Driver() : public rclcpp::Node
{
    ...
    {
        RCLCPP_INFO_STREAM_ONCE(node->get_logger(),"Main board Serial Port open success, com_port_name= " << serial_port_);
    }
    else
    {
    ...
    if (stream->version_detection())
    {
        ...
        RCLCPP_INFO_STREAM_ONCE(node->get_logger(),"The version matches successfully, current version: [" << (int)version.protoVer << "]");
        RCLCPP_INFO_STREAM_ONCE(node->get_logger(),"GET Equipment Identity: " << version.equipmentIdentity);
    }
    else
    {
        Data_Format_VER version = stream->get_data_version();
...
void Base_Driver::init_robot_stream()
{
    ...
    ros::SubscriberStatusCallback status_cb = std::bind( [&]()
    {
        if (pub_bat_.getNumSubscribers() > 0)
        {
            ...
            RCLCPP_INFO_STREAM_ONCE(node->get_logger(),"Starting battery data stream.");
        }
        else 
        {
            bmsStreamActive = false;
            RCLCPP_INFO_STREAM_ONCE(node->get_logger(),"Stopping battery data stream.");
        }
    });
    pub_bat_ = nh_.advertise<lingao_msgs::LingAoBmsStatus>("battery_state", 1, status_cb, status_cb);
    RCLCPP_INFO_STREAM_ONCE(node->get_logger(),"advertise to the battery state topic on [" << pub_bat_.getTopic() << "]");
    ...
    if(stream->rcAvailable() == true)
    {
        ros::SubscriberStatusCallback status_cb = std::bind( [&]()
        {
            ...
            {
                rcStreamActive = true;
                RCLCPP_INFO_STREAM_ONCE(node->get_logger(),"Starting RC data stream.");
            }
            else 
            {
                rcStreamActive = false;
                RCLCPP_INFO_STREAM_ONCE(node->get_logger(),"Stopping RC data stream.");
            }
        });

        pub_rc_ = nh_.advertise<lingao_msgs::LingAoRCStatus>("rc_state", 1, status_cb, status_cb);
        RCLCPP_INFO_STREAM_ONCE(node->get_logger(),"advertise to the rc state topic on [" << pub_rc_.getTopic() << "]");
    }
}
    ...
    if(use_imu_ == true) 
    {
        ...
        ros::SubscriberStatusCallback status_cb = std::bind( [&]()
        {
            if (pub_imu_.getNumSubscribers() > 0)
            {
                ...
                RCLCPP_INFO_STREAM_ONCE(node->get_logger(),"Starting IMU data stream.");
            }
            else
            {
                imuStreamActive = false;
                RCLCPP_INFO_STREAM_ONCE(node->get_logger(),"Stopping IMU data stream.");
            }
        });

        pub_imu_ = nh_.advertise<sensor_msgs::Imu>(topic_imu_, 1, status_cb, status_cb);
        RCLCPP_INFO_STREAM_ONCE(node->get_logger(),"advertise to the imu topic on [" << pub_imu_.getTopic() << "]");
    }
}
...
void Base_Driver::init_odom()
{
    ...
    RCLCPP_INFO_STREAM_ONCE(node->get_logger(),"advertise to the odom topic on [" << pub_odom_.getTopic() << "]");

    sub_cmd_vel_ = nh_.subscribe(topic_cmd_vel_name_, 1, &Base_Driver::cmd_vel_CallBack, this);
    RCLCPP_INFO_STREAM_ONCE(node->get_logger(),"subscribe to the cmd topic on [" << sub_cmd_vel_.getTopic() << "]");
